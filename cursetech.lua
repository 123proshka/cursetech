local requirements = {
    ...
    obex_data = obex_fetch and obex_fetch() or {username = 'admin', build = 'alpha'},
...
ui.new_label("AA", "Anti-aimbot angles", "»  ~\aE1DEFD99 "..build)
...
local menu = {
    ...
    hometext = ui.new_label("aa", "anti-aimbot angles"," ~ \aE1DEFD99join our discord below"),
    discords = ui.new_button("aa", "anti-aimbot angles", "our discord" , function()
        panorama.loadstring("SteamOverlayAPI.OpenExternalBrowserURL('https://discord.gg/sS4ykzSjAe');")()
    end),
...
function watermark_curse()
    local x, y = client_screen_size()
    local mx, my = renderer_measure_text("b", "curse.tech")
    ...
    renderer_text(22+32+7, y/2+3, 235, 235, 235, 255*alpha, "b", 0, string.format("\a%sc \a%su \a%sr \a%ss \a%se \a%s. \a%st \a%se \a%sc \a%sh", RGBAtoHEX(unpack(aA[1])), RGBAtoHEX(unpack(aA[2])), RGBAtoHEX(unpack(aA[3])), RGBAtoHEX(unpack(aA[4])), RGBAtoHEX(unpack(aA[5])), RGBAtoHEX(unpack(aA[6])), RGBAtoHEX(unpack(aA[7])), RGBAtoHEX(unpack(aA[8])), RGBAtoHEX(unpack(aA[9])),RGBAtoHEX(unpack(aA[10])),RGBAtoHEX(unpack(aA[11])),RGBAtoHEX(unpack(aA[12])),RGBAtoHEX(unpack(aA[13])),RGBAtoHEX(unpack(aA[14])),RGBAtoHEX(unpack(aA[15])),RGBAtoHEX(unpack(aA[16])),RGBAtoHEX(unpack(aA[17]))))
    renderer_text(22+32+19, y/2+19, 235, 235, 235, 255*alpha, "c", 0, user)
end

client.set_event_callback("paint", watermark_curse)
...
elseif ui.get(antibrute[brute.phase].jitter) == "5 Way" then
    ...
    ui.set(ref.jitter[2], side == 1 and ui.get(antibrute[brute.phase].left_jitter_val) or ui.get(antibrute[brute.phase].right_jitter_val))
            ...
            renderer.gradient(center[1], 100, 200, 50, 255, 0, 0, 255, 0, 0, 255, 255, true)
                    ...
                    renderer.text(x/2-25+aaxs, y/2+8, r, g, b, a *alpha, 'b', 0,  string.format("\a%s   c\a%su\a%sr\a%ss\a%se", RGBAtoHEX(unpack(aA[1])), RGBAtoHEX(unpack(aA[2])), RGBAtoHEX(unpack(aA[3])), RGBAtoHEX(unpack(aA[4])), RGBAtoHEX(unpack(aA[5])), RGBAtoHEX(unpack(aA[6])), RGBAtoHEX(unpack(aA[7])), RGBAtoHEX(unpack(aA[8])), RGBAtoHEX(unpack(aA[9])),RGBAtoHEX(unpack(aA[10])),RGBAtoHEX(unpack(aA[11])),RGBAtoHEX(unpack(aA[12])),RGBAtoHEX(unpack(aA[13])),RGBAtoHEX(unpack(aA[14])),RGBAtoHEX(unpack(aA[15])),RGBAtoHEX(unpack(aA[16])),RGBAtoHEX(unpack(aA[17]))))
                    ...
                    renderer.text(x/2+aaxs2, y/2+24-vsss, ca, ca2, ca3, expalpha*alpha, flag, 0, name)
                        ...
                        local text = "curse " .. requirements.obex_data.build:upper()
                        local text_width, text_height = renderer.measure_text(requirements.obex_data.build:upper(), "curse ")
